   ...
   fUpdateCmd(0)
{
  ...
  fLocalMagFieldCmd = new G4UIcmdWithADoubleAndUnit("/field/setLocalField",this);
  fLocalMagFieldCmd->SetGuidance("Define local magnetic field.");
  fLocalMagFieldCmd->SetParameterName("Gradient" ,false,false);
...
void F03FieldMessenger::SetNewValue( G4UIcommand* command, G4String newValue)
{
  ...
  {
    fEMfieldSetup->SetLocalFieldValue(fLocalMagFieldCmd->GetNewDoubleValue(newValue), G4ThreeVector(0.,0.,0.), 0.*deg);
    fEMfieldSetup->UpdateField();
    // Check the value
