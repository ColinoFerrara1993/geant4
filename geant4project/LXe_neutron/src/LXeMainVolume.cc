...
LXeMainVolume::LXeMainVolume(G4RotationMatrix* pRot, const G4ThreeVector& tlate,
  ...
  : G4PVPlacement(pRot, tlate,
                  ...
                  new G4LogicalVolume(new G4Box("temp1", 30.*cm, 30.*cm, 50.*cm),
                                      G4Material::GetMaterial("Vacuum"), "temp1",
                                      0, 0, 0),
                  "housing1", pMotherLogical, pMany, pCopyNo)
  , fConstructor(c)
{
  CopyValues();
  ...
  G4double moderator_x = fScint_x + 2. * fD_mtl + 2. * fD_moderator;
  G4double moderator_y = fScint_y + 2. * fD_mtl + 2. * fD_moderator;
  G4double moderator_z = fScint_z + 2. * fD_mtl + 2. * fD_moderator;

  //*************************** housing and scintillator and moderator
  fScint_box =
    new G4Box("scint_box", fScint_x / 2., fScint_y / 2., fScint_z / 2.);
  fHousing_box =
    new G4Box("housing_box", housing_x / 2., housing_y / 2., housing_z / 2.);
  fModerator_box =
    new G4Box("moderator_box", moderator_x / 2., moderator_y / 2., moderator_z / 2.);
  

  fScint_log   = new G4LogicalVolume(fScint_box, G4Material::GetMaterial("GlassSci"), //Glass Scintillator Lithium Cerium
                                   "scint_log", 0, 0, 0);
  fHousing_log = new G4LogicalVolume(fHousing_box, G4Material::GetMaterial("Al"), "housing_log", 0, 0, 0);
    
  fModerator_log = new G4LogicalVolume(fModerator_box, G4Material::GetMaterial("Pethylene1"), "moderator_log", 0, 0, 0);

  new G4PVPlacement(0, G4ThreeVector(), fScint_log, "scintillator", fHousing_log, false, 0);
  
  new G4PVPlacement(0, G4ThreeVector(), fHousing_log, "moderator", fModerator_log, false, 0);
  ...
  SetLogicalVolume(fModerator_log);
}
...
void LXeMainVolume::CopyValues()
{
  ...
  fD_moderator     = fConstructor->GetModeratorThickness();
}
...
void LXeMainVolume::VisAttributes()
{
  ...
  G4VisAttributes* moderator_va = new G4VisAttributes(G4Colour(0.8, 0.8, 0.8));
  fModerator_log->SetVisAttributes(moderator_va);
}
