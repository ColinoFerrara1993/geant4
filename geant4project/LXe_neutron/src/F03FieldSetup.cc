   ...
   fFieldMessenger(0)
{
  fMagneticField = new G4UniformMagField(G4ThreeVector(0.0*tesla,
                                                       0.0, // 0.5*tesla,
                                                       0.0));
  fLocalMagneticField = new G4QuadrupoleMagField( (0.*tesla)/(1.0*m) );
  ...
  fMinStep     = 0.000000001*mm ; // minimal step of 1 mm is default
  fStepperType = 11 ;       // ClassicalRK4 is default stepper
...
void F03FieldSetup::CreateSteppers()
{
  ...
  switch ( fStepperType )
  {
    ...
    case 10:
      ...
      fLocalStepper = new G4DormandPrince745( fLocalEquation );
      G4cout<<"G4DormandPrince745 is called"<<G4endl;
      break;
    case 11:
      fStepper = new G4DormandPrinceRK56( fEquation );
      fLocalStepper = new G4DormandPrinceRK56( fLocalEquation );
      G4cout<<"G4DormandPrinceRK56 is called"<<G4endl;
      break;
    default: fStepper = 0;
  }
}
...
void F03FieldSetup::SetLocalFieldValue(G4double fieldGradient, G4ThreeVector fieldpos, G4double theta)
{
  ...
  if(fieldGradient != 0.)
  {
    ...
    fLocalMagneticField = new G4QuadrupoleMagField(fieldGradient, fieldpos, matrix);
  }
  else
  {
    // If the new field's value is Zero, then
