...
void LXeTrajectory::DrawTrajectory() const
{
  ...
  if(lineRequired)
  {
    ...
    if(fParticleDefinition == G4OpticalPhoton::OpticalPhotonDefinition())
    {
      ...
      {  // Scintillation and Cerenkov photons are green
    ...
    else  if(fParticleDefinition->GetParticleName() == "proton"){
    
            colour = (G4Colour(0., 1., 1.)); // Magenta
    
    } else if(fParticleDefinition->GetParticleName() == "neutron"){
        ...
        } else if(fParticleDefinition->GetParticleName() == "e-"){
