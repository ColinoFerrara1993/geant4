...
void LXeRun::Merge(const G4Run* run)
{
  ...
  fNthreshold += localRun->fNthreshold;
...
void LXeRun::EndOfRun()
{
  ...
  G4double rms_edep = edep2 - edep*edep/n_evt;
  if (rms_edep > 0.) rms_edep = std::sqrt(rms_edep); else rms_edep = 0.;  
  ...
  G4double rmsDose = rms_edep/mass;
  
  G4cout << "Total dose deposition in scoring volume:\t " << G4BestUnit(dose, "Dose")
         << " +- " << G4BestUnit(rmsDose, "Dose") << G4endl;
  G4cout << G4endl;
  G4cout.precision(prec);
  
  LXeEventAction* evt = new LXeEventAction(detectorConstruction);
  G4int th1 = evt->Getthreshold1();
  G4int th2 = evt->Getthreshold2();
  G4cout << "Events with a number of hits between " << th1 << " and " << th2 << " hits are " << fNthreshold << G4endl;
  G4cout << G4endl;
}
