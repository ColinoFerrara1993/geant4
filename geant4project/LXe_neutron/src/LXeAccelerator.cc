...
LXeAccelerator::LXeAccelerator(G4RotationMatrix* pRot, const G4ThreeVector& tlate,
                             G4LogicalVolume* pMotherLogical, G4bool pMany,
                             G4int pCopyNo, LXeDetectorConstruction* c)
  // Pass info to the G4PVPlacement constructor
  : G4PVPlacement(pRot, tlate,
                  // Temp logical volume must be created here
                  new G4LogicalVolume(new G4Box("temp2", 1.*cm, 1.*cm, 1.*cm),
                                      G4Material::GetMaterial("Vacuum"), "temp2",
                                      0, 0, 0),
                  "housing2", pMotherLogical, pMany, pCopyNo)
  , fConstructor(c)
{
  ...
  // Envelope parameters
  //
  G4double env_sizeXY = 20.*cm, env_sizeZ = (0.99+0.80)*m;
  ...
  G4double shape0_rmin =  3.*mm, shape0_rmax = 1.*cm;
  G4double shape0_hz = 1.*cm;
  G4double shape0_phimin = 0.*deg, shape0_phimax = 360.*deg;
  G4ThreeVector pos0 = G4ThreeVector(0*cm, 0*cm, -20.*cm);

  G4Tubs* solidShape0 =    
    new G4Tubs("Shape0",                     //its name
      shape0_rmin, shape0_rmax, shape0_hz, shape0_phimin, shape0_phimax);//its size
  ...
  //Position
  G4ThreeVector pos1 = pos0;
     
  G4double shape1_rmin =  0.*mm, shape1_rmax = 3.*mm;
  ...
  // Beam pipe envelope
  //
  G4Material* shape_mat = nist->FindOrBuildMaterial("G4_STAINLESS-STEEL");
  G4double shape_rmin =  3.*mm, shape_rmax = 3.2*mm;
  G4double shape_hz = 3.25*cm;
  G4double shape_phimin = 0.*deg, shape_phimax = 360.*deg;
  
  //Position
  G4ThreeVector pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + shape_hz);
  ...
  // Shape 2 - Beam pipe 2
  //
  G4double shape2_rmin =  0.*mm, shape2_rmax = 3.*mm;
  G4double shape2_hz = 3.25*cm;
  G4double shape2_phimin = 0.*deg, shape2_phimax = 360.*deg;
  
  //Position
  G4ThreeVector pos2 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + shape2_hz);
  ...
  G4double layer = 0.*mm;
  //
  // Shape 3 - Quadrupole 2
  //
  G4double shape3_rmin =  3.*mm, shape3_rmax = 1.*cm;
  G4double shape3_hz = 1.*cm;
  G4double shape3_phimin = 0.*deg, shape3_phimax = 360.*deg;
  G4ThreeVector pos3 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + shape3_hz);
  G4Tubs* solidShape3 =    
    new G4Tubs("Shape3", 
    shape3_rmin, shape3_rmax, shape3_hz, shape3_phimin, shape3_phimax);
  ...
  // Shape 4 - Beam pipe 3
  //
  G4double shape4_rmin =  0.*mm, shape4_rmax = 3.*mm;
  G4double shape4_hz = 1.*cm;
  G4double shape4_phimin = 0.*deg, shape4_phimax = 360.*deg;
  G4ThreeVector pos4 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + shape3_hz);
  G4Tubs* solidShape4 =    
    new G4Tubs("Shape4", 
    shape4_rmin, shape4_rmax, shape4_hz, shape4_phimin, shape4_phimax);
  ...
  G4double shape5_rmin =  1.5*mm, shape5_rmax = 3.*cm;
  G4double shape5_hz = 3.*cm;
  G4double shape5_phimin = 0.*deg, shape5_phimax = 360.*deg;
  G4ThreeVector pos5 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + shape5_hz );
  G4Tubs* solidShape5 =    
  ...
  // Shape 6 - Beam pipe 4
  //
  G4double shape6_rmin =  0.*mm, shape6_rmax = 1.5*mm;
  G4double shape6_hz = 3.*cm;
  G4double shape6_phimin = 0.*deg, shape6_phimax = 360.*deg;
  G4ThreeVector pos6 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + shape5_hz);
  G4Tubs* solidShape6 =    
  ...
  //Position
  G4ThreeVector beampipe1_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + beampipe1_hz);
  ...
  //Position
  G4ThreeVector vacuum1_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + vacuum1_hz);
  ...
  G4ThreeVector pos7 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + shape7_hz);
  ...
  G4ThreeVector pos8 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + shape7_hz);
  ...
  //Position
  G4ThreeVector beampipe2_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + beampipe2_hz);
  ...
  //Position
  G4ThreeVector vacuum2_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + vacuum2_hz);
  ...
  G4double shape9_rmin =  1.5*mm, shape9_rmax = 3.*cm;
  G4double shape9_hz = 3.*cm;
  G4double shape9_phimin = 0.*deg, shape9_phimax = 360.*deg;
  G4ThreeVector pos9 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*vacuum2_hz + shape9_hz);
  G4Tubs* solidShape9 =    
  ...
  //
  G4double shape10_rmin =  0.*mm, shape10_rmax = 1.5*mm;
  G4double shape10_hz = 3.*cm;
  G4double shape10_phimin = 0.*deg, shape10_phimax = 360.*deg;
  G4ThreeVector pos10 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*vacuum2_hz + shape9_hz);
  G4Tubs* solidShape10 =    
  ...
  //Position
  G4ThreeVector beampipe3_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*beampipe2_hz + 2*shape9_hz + beampipe3_hz);
  ...
  //Position
  G4ThreeVector vacuum3_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*vacuum2_hz + 2*shape9_hz + vacuum3_hz);
  ...
  G4ThreeVector pos11 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*vacuum2_hz + 2*shape9_hz + 2*vacuum3_hz + shape11_hz);
  ...
  G4ThreeVector pos12 = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*vacuum2_hz + 2*shape9_hz + 2*vacuum3_hz + shape11_hz);
  ...
  //Position
  G4ThreeVector beampipe4_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*beampipe2_hz + 2*shape9_hz + 2*beampipe3_hz + 2*shape11_hz + beampipe4_hz);
  ...
  //Position
  G4ThreeVector vacuum4_pos = G4ThreeVector(0*cm, 0*cm, -20.*cm + shape1_hz + 2*shape2_hz + layer + 2*shape3_hz + layer + 2*shape5_hz + 2*vacuum1_hz + 2*shape7_hz + 2*vacuum2_hz + 2*shape9_hz + 2*vacuum3_hz + 2*shape11_hz + beampipe4_hz);
